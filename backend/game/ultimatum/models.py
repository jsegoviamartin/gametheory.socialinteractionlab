...
class UltimatumGameRound(models.Model):
    ...
    endowment = models.IntegerField(default=10)
    ...
    def delete_incomplete_match(cls, match_uuid):
        """Delete incomplete match if less than total_rounds completed"""
        first = cls.objects.filter(game_match_uuid=match_uuid, round_number=1).first()
        if not first: return False
        
        completed_count = cls.get_completed_rounds_count(match_uuid)
        if completed_count < first.total_rounds:
