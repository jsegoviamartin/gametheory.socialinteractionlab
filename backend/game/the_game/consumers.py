...
class GameConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        # ─── actions ───
        if action in (self.game_match.label_a, self.game_match.label_b, "Cooperate", "Defect", "Prize Fight", "Ballet", "Stag", "Hare"):
            if fp not in (
    ...
    def process_action(self, fp, action) -> bool:
        ...
        if rnd_no > self.game_match.total_rounds:
            return False
    ...
    def calculate_round_results(self):
        r = self.game_match.rounds.order_by("-round_number").first()
        p1, p2 = calculate_payoff(r.player_1_action, r.player_2_action, self.game_match)
        ...
        if r.round_number < self.game_match.total_rounds:
    ...
    def get_game_state(self):
        ...
        game_over = self.game_match.is_complete or next_rnd > self.game_match.total_rounds
        waiting = not bool(self.game_match.player_2_fingerprint) \
                    and self.game_match.game_mode == "online"

        last = rounds.last()
        return {
            "currentRound": min(next_rnd, self.game_match.total_rounds),
            "maxRounds": self.game_match.total_rounds,
            "player1Score": p1,
            "player2Score": p2,
            ...
            "gameType": self.game_match.game_type,
            "roomName": self.game_match.room_name,
            "labelA": self.game_match.label_a,
            "labelB": self.game_match.label_b,
            "p1_cc": self.game_match.p1_cc, "p2_cc": self.game_match.p2_cc,
            "p1_cd": self.game_match.p1_cd, "p2_cd": self.game_match.p2_cd,
            "p1_dc": self.game_match.p1_dc, "p2_dc": self.game_match.p2_dc,
            "p1_dd": self.game_match.p1_dd, "p2_dd": self.game_match.p2_dd,
            "player1Fingerprint": self.game_match.player_1_fingerprint,
            "player2Fingerprint": self.game_match.player_2_fingerprint,
            "experimentId": str(self.game_match.experiment_id) if self.game_match.experiment_id else None,
        }
    ...
    def save_final_scores(self, gs):
        ...
        self.game_match.save()
        # Export completed matches immediately after the final round
        export_prisoner_all()
    ...
    async def make_bot_move(self):
        ...
        bot_action = await sync_to_async(make_bot_decision)(
            player_hist, bot_hist, self.game_match.label_a, self.game_match.label_b
        )
